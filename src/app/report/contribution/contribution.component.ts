...
} from "../../income-and-expenditure/contribution-payment/contribution-payment.service";
...
import {UploadComponent} from "../../income-and-expenditure/contribution-payment/upload/upload.component";
...
export class ContributionComponent implements OnInit {
  ...
  download(format: string): void {
    if (format === 'PDF') {
      this.contributionPaymentService.download(format,this.activatedHierarchy.id,this.queryString,this.start,this.end,this.products).subscribe(response => {
        saveAs(new Blob([response], {type: 'application/pdf'}), Date.now() + '-contributions.pdf');
      });
    } else {
      this.contributionPaymentService.download(format,this.activatedHierarchy.id,this.queryString,this.start,this.end,this.products).subscribe(response => {
